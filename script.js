...
function openStudyLibrary(mode, diff) {
    ...
    document.getElementById('study-title').innerText = quizData[mode].title + (diff === 'exact' ? " (Exact Translation)" : ` (${diff.toUpperCase()})`);
...
function returnToMenu() {
    ...
    const screens = ['login-screen', 'home-screen', 'study-screen', 'quiz-screen', 'result-screen', 'leaderboard-screen', 'jeopardy-screen', 'difficulty-modal'];
    screens.forEach(id => {
...
socket.on('receive_chat', (data) => {
    ...
    if (isSystem) {
        ...
        let bubble = `<div style="background:${isMe ? 'rgba(212,175,55,0.15)' : '#222'}; border:${isMe ? '1px solid rgba(212,175,55,0.4)' : '1px solid #333'}; padding:10px 15px; border-radius:12px; max-width:80%;">
